...
class Player {
  ...
  loadEvents() {
    ...
    this.audio.onended = async () => {
      if (previousVerseText) {
        ...
        this.players.points += finalResult.points;
        previousVerseText = "";
      }
  ...
  if (previousVerseText) {
    // pequena folga pra deixar o reconhecimento "alcançar" o áudio
      ...
      boxes.forEach((box, playerIndex) => {
        ...
        if (targetVerse) {
          ...
          targetVerse.scrollIntoView({
            behavior: 'smooth',
            block: 'center' // Garante que o verso fique no centro do container visível
          });
        }
      });
    });
